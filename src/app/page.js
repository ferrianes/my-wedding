    ...
    <div>
      ...
      <div className="flex w-max mx-auto">
        <div className="w-[200px]">
          <TypingEffect key={1} text="Chintia" speed={200} className={`${alexBrush.variable} font-alex-brush text-6xl text-[#5A5F4F] text-left`} animationVariant={animationVariantsTextLeft} />

          <TypingEffect key={2} text="&" speed={200} className={`${alexBrush.variable} font-alex-brush text-4xl leading-[0.6] text-[#AA6C42] text-center`} animationVariant={animationVariantsTextCenter} />

          <TypingEffectBackward key={3} text="Ferrian" speed={200} className={`${alexBrush.variable} font-alex-brush text-6xl text-[#5A5F4F] text-right`} animationVariant={animationVariantsTextRight} />
        </div>
      </div>
      <div className="flex flex-col mt-4">
        <TypingEffect key={4} text="Dear:" speed={100} className={`${marcellus.variable} font-marcellus text-2xl text-[#5A5F4F] text-center`} animationVariant={animationVariantsTextLeft} />
        <TypingEffectBackward key={5} text="xxx" speed={100} className={`${marcellus.variable} font-marcellus text-2xl text-[#5A5F4F] text-center`} animationVariant={animationVariantsTextRight} />
      </div>
      <div className="text-center mt-4 z-50 relative">
        <Link href="/intro">
          <motion.button type="button" className="text-white bg-gradient-to-r from-stone-500 via-stone-600 to-stone-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-stone-300 shadow-lg shadow-stone-500/50 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2" variants={animationVariantsFadeLeft} initial="initial" animate="animate">
            <TypingEffect key={6} text="Open Invitation" speed={200} animationVariant={animationVariantsTextLeft} />
          </motion.button>
        </Link>
      </div>
      <motion.div className='' variants={animationVariantsFadeUp} initial="initial" animate="animate">
